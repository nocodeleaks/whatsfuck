...
import (
	...
	"github.com/nocodeleaks/whatsfuck/proto/waCert"
...
//
// On success, it returns the handler queue for this connection. The queue is created fresh for
// every connection (rather than reused from the Client) so that nodes queued by a previous,
// since-disconnected connection can never bleed into the new one.
