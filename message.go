...
import (
	...
	"github.com/nocodeleaks/whatsfuck/proto/waLidMigrationSyncPayload"
...
func (cli *Client) parseMessageSource(node *waBinary.Node, requireParticipant bool) (source types.MessageSource, err error) {
	...
	if from.Server == types.GroupServer || from.Server == types.BroadcastServer {
		...
		// The explicit sender domain determines the opposite identity. A phone
		// participant must retain its provided LID instead of repeating its PN,
		// even when the addressing-mode attribute is absent or inconsistent.
		phoneSender := source.Sender.Server == types.DefaultUserServer || source.Sender.Server == types.HostedServer
		if phoneSender || (source.SenderAlt.IsEmpty() && source.AddressingMode != types.AddressingModeLID) {
